...
type Programme = {
  id: string; imdbId?: string; type: "show" | "movie"; title: string; poster?: string; releaseInfo?: string;
...
export function LegacyParentWorkflows({ secret, surface }: { secret: string; surface: "programmes" | "settings" }) {
  ...
  useEffect(() => {
    if (surface === "programmes") void loadLibrary();
  }, [loadLibrary, surface]);
  ...
  return <div className="legacy-workflows">
    {surface === "programmes" && <section aria-labelledby="legacy-library-heading">
      ...
      <div id="library" className="programme-grid">{library.length ? library.map(programme => <LibraryProgramme key={programme.id} programme={programme} base={base} act={libraryAction} />) : <p>No programmes approved yet.</p>}</div>
    </section>}
...
function LibraryProgramme({ programme, base, act }: { programme: Programme; base: string; act: (path: string, method: string, body?: unknown) => Promise<void> }) {
  const [episodes, setEpisodes] = useState(programme.episodes ?? []);
  const [episode, setEpisode] = useState(programme.showProgress?.id || programme.episodes?.[0]?.id || "");

  useEffect(() => {
    if (programme.type !== "show" || programme.episodes) return;
    let active = true;
    void fetch(`${base}/cinemeta/title/show/${encodeURIComponent(programme.imdbId ?? programme.id)}`, { cache: "no-store" })
      .then(async response => ({ response, result: await resultOf(response) }))
  ...
  return <ProgrammeCard programme={programme}>{programme.type === "show" && <><p>{programme.showProgress ? `Show Progress: ${episodeLabel(programme.showProgress)}` : "Finished"}</p><select aria-label={`Next episode for ${programme.title}`} value={episode} onChange={event => setEpisode(event.target.value)}>{episodes.map(item => <option key={item.id} value={item.id}>{episodeLabel(item)}</option>)}</select><Button type="button" disabled={!episode} onClick={() => void act(`/library/${programme.id}/progress`, "PATCH", { videoId: episode })}>{programme.showProgress ? "Set Show Progress" : "Restart show"}</Button><Button type="button" onClick={() => void act(`/library/${programme.id}`, "PATCH", { paused: !programme.pausedAt })}>{programme.pausedAt ? "Resume show" : "Pause show"}</Button><Button type="button" className="button-secondary" onClick={() => void act(`/library/${programme.id}`, "DELETE")}>Remove show</Button></>}</ProgrammeCard>;
}
